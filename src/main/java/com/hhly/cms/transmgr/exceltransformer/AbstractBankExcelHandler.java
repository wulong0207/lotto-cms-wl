...
public abstract class AbstractBankExcelHandler implements IBankExcelHandler {
	...
	public void validateTakenTrans(String bankId) {
		...
		for(int i = 1 ; i < excels.size(); i++) {
			...
			List<com.hhly.skeleton.cms.transmgr.bo.TransTakenBO> takens = transMgrService.findTakenTransNoPaging(vo2);
			Assert.isTrue(!CollectionUtils.isEmpty(takens), String.format("不存在流水号为:%s的提款记录", transTakenCode));

			com.hhly.skeleton.cms.transmgr.bo.TransTakenBO taken = takens.get(0);
			// 2017-12-19 运营人员说：原路返回时，当是退款到微信或支付宝，则只有银行名称，没有账号，故可为空,所以在账号不为空时，才验证账号是否正确
			String bankAccount = row.get(getBankAccountIndex()).trim();
			if(StringUtils.hasText(taken.getBankCardNum())) {
				vo.setBankCardNum(bankAccount);
			}
			double realAmount = Double.parseDouble(row.get(getRealAmountIndex()).replace(",","").trim());
			...
			int count = transMgrService.countTransTaken(vo);
