...
<html>
  ...
  <div class="mini-toolbar" style="border-top:0;border-left:0;padding:0px;">
		...
		<table style="width:100%;">
			<tr>
				<td width="50%" nowrap="nowrap">
					<btn:operate privilege="CHECK_TAKEN">
				        <a class="mini-button" iconCls="icon-edit" plain="true" onclick="trans_taken.toCheckWindow">审核提款</a>
			        </btn:operate>
					<btn:operate privilege="PROCESS_TAKEN">
				        <a class="mini-button" iconCls="icon-upload" plain="true" onclick="trans_taken.toProcessWindow">处理提款</a>
			        </btn:operate>
					<btn:operate privilege="ALREADY_PROCESS_TAKEN">
				        <a class="mini-button" iconCls="icon-edit" plain="true" onclick="trans_taken.toAlreadyProcessWindow">已处理提款</a>
			        </btn:operate>
					<btn:operate privilege="ALREADY_PROCESS_TAKEN">
				        <a class="mini-button" iconCls="icon-edit" plain="true" onclick="trans_taken.batchAlreadyProcess">批量已处理提款</a>
			        </btn:operate>
			         <%--
					<btn:operate privilege="PROCESS_SUCCESS_TAKEN">
				        <a class="mini-button" iconCls="icon-edit" plain="true" onclick="trans_taken.toProcessSuccessWindow">银行处理成功提款</a>
			        </btn:operate>
					<btn:operate privilege="TAKEN_CONFIG">
				        <a class="mini-button" iconCls="icon-edit" plain="true" onclick="trans_taken.">提款配置</a>
			        </btn:operate>
			        --%>
			        <btn:operate privilege="EXPORT">
				        <a class="mini-button" iconCls="icon-download" plain="true" onclick="trans_common.excel('transmgr/taken')">导出Excel</a>
			        </btn:operate>
			        <btn:operate privilege="EXPORT">
				        <a class="mini-button" iconCls="icon-download" plain="true" onclick="trans_taken.showExcelBank">导出银行提款Excel</a>
				        <%--
				        <input id="exportExcelBank" class="mini-combobox" style="width:150px;"
					emptyText="选择导出银行"  valueFromSelect ="true" showClose="true" oncloseclick="Cms.onCloseClick" onvaluechanged="trans_taken.bankExcel('transmgr/taken/bank')"/>
			        --%>
			        </btn:operate>
				</td>
			</tr>
			<tr>
				<td>
					...
					提款分类<input name="takenType" class="mini-combobox" data="[{id:1,text:'正常提款'},{id:2,text:'原路返回'}]"
					emptyText="请选择" valueFromSelect ="true" showClose="true" oncloseclick="Cms.onCloseClick"/>
				</td>
			</tr>
			<tr>
				<td>
					...
					emptyText="请选择"  valueFromSelect ="true" showClose="true" oncloseclick="Cms.onCloseClick"/>
					<a class="mini-button" onclick="trans_common.search(trans_taken.grid,'form1')" >查询</a>
				</td>
			</tr>
		</table>
	</div>
	...
	<div id="datagrid" class="mini-datagrid" onpreload="MiniCom.onpreload" allowAlternating="true" borderStyle="border:solid 0px;" style="width:100%;height:90%;" 
		 url="transmgr/taken/list" idField="id" allowResize="true" multiSelect="true" 
		 showColumnsMenu="true" onrowdblclick="trans_taken.viewDetail" showFilterRow="false" showSummaryRow = "true" ondrawsummarycell="trans_taken.onDrawSummaryCell" sizeList="[10,20,50,100,500,1000]" pageSize=100>
		<div property="columns">
			...
			<div field="dealTime" headerAlign="center" align="center" dateFormat="yyyy-MM-dd HH:mm:ss">银行处理时间</div>
			<div field="arrivalTime" headerAlign="center" align="center" dateFormat="yyyy-MM-dd HH:mm:ss">到账时间</div>
			<div field="thirdTransNum" headerAlign="center" align="center">第三方流水号</div>
			<div field="batchNum" headerAlign="center" align="center">批次号</div>
...
<!-- 	                <td style="width:150px;"><input name="createTime" class="mini-datepicker" enabled="false" format="yyyy-MM-dd HH:mm:ss"/></td> -->
	...
	<div id="checkTakenWindow" class="mini-window" title="审核提款" style="width:500px;height:400px;" onbuttonclick="trans_taken.clearCheckTakenWindow">
...
<button type="button" class="btn btn-info btn-sm"
		...
		<%--
		驳回原因<input id="rejectReason" class="mini-textbox"/></br>
		--%>
	...
	<%--
	<div id="processTakenWindow" class="mini-window" title="处理提款" style="width:500px;height:400px;">
		是否处理选中的提款请求</br>
		<button type="button" class="btn btn-info btn-sm" id="confirmProcessBtn">确定</button>
		<button type="button" class="btn btn-info btn-sm" id="cancelProcessBtn">取消</button>
		<table class="table table-bordered table-sm">
		  <tbody>
		    <tr>
                <td>创建时间</td>
	...
	<div id="processTakenWindow" class="mini-window" title="处理提款"
		...
		onbuttonclick="trans_taken.clearProcessTakenWindow">
		<div style="text-align:center;">
			...
			emptyText="选择银行模板" valueFromSelect ="true" showClose="true" oncloseclick="Cms.onCloseClick" required="true"/>
				    	...
				    	<a class="mini-button" onclick="trans_taken.preUploadBankExcel">上传</a>
				    	</btn:operate>
					</form>
			</div>
			...
			<div class="row">
				...
				<%--
				<div style="padding: 10px 10px 0 50px;" class="text-center">
					<table class="table table-bordered table-sm" id="processSuccessTaken"
				...
				<div style="padding: 10px 10px 0 10px;">
					<table class="table table-bordered table-sm" id="processFailTaken"
						data-toggle="tooltip" title="双击移动记录">
						<thead class="thead-default">
							<tr>
								<th>序号</th>
	...
	<div id="alreadyProcessTakenWindow" class="mini-window" title="已处理提款"
		style="width: 770px; height: 600px;"
		onbuttonclick="trans_taken.clearAlreadyProcessTakenWindow">
		<div style="text-align:center;">
			<div class="input-group input-group-sm col-sm-6" style="margin-left : 180px;">
				<span class="input-group-addon" id="sizing-addon">批次号</span> <input
	...
	<div id="processSuccessTakenWindow" class="mini-window" title="银行处理成功提款"
		style="width: 770px; height: 600px;"
		onbuttonclick="trans_taken.clearProcessSuccessTakenWindow">
		<div style="text-align:center;">
			<div class="input-group input-group-sm col-sm-6" style="margin-left : 180px;">
				<span class="input-group-addon" id="sizing-addon">批次号</span> <input
					type="text" class="form-control" placeholder="请输入批次号"
...
<script src="<%=basePath%>resources/js/transmgr/trans_taken.js" type="text/javascript"></script>
</html>
