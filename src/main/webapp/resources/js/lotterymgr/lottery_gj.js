var lottery_gj = {
	init : function() {
		mini.parse();
        lottery_gj.grid = mini.get('datagrid');
        lottery_gj.lotteryIssue = mini.get('lotteryIssue');
        $.get("sysmgr/dic/dictionary?code=0507", function(result){
            ...
            ComReq.issue(308,function(result){
                lottery_gj.lotteryIssue.setData(result);
                lottery_gj.lotteryIssue.select(0);
                lottery_gj.search();
            });
        });
	},
    search : function() {
        ...
        data.lotteryCode = 308;
        lottery_gj.grid.load(data);
    },
    onActionRenderer:function(e){
        ...
        if (grid.isEditingRow(record)) {
            var s = '<a class="mini-button"  plain="true" href="javascript:lottery_gj.updateRow(\'datagrid\',\'' + uid + '\')" >保存</a>&nbsp;&nbsp;&nbsp;'+
    ...
    draw:function(){
        ...
        param.data = {'jobId':'000020','lotteryCode':'308','lotteryIssue':lottery_gj.lotteryIssue.value};
        var message = "确定是否执行 <span style='color:red'>开奖</span>?";
        Cms.ajaxFirm(message, param, function (data) {
            Cms.drawSchedule(308, lottery_gj.lotteryIssue.value, 1, () => {}
            )
            ;
        })
    }
